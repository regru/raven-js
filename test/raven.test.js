...
describe('globals', function() {
  ...
  describe('trimPacket', function() {
    it('should work as advertised', function() {
      ...
      assert.deepEqual(Raven._trimPacket({ message: 'lol' }), {
        message: 'lol'
      });
      assert.deepEqual(Raven._trimPacket({ message: 'lolol' }), {
        message: 'lol\u2026'
      });
      assert.deepEqual(
        Raven._trimPacket({
          message: 'lol',
          exception: { values: [{ value: 'lol' }] }
        }),
  ...
  describe('processException', function() {
    ...
    it('should respect `ignoreUrls`', function() {
      ...
      Raven._globalOptions.ignoreUrls = joinRegExp([
        /.+?host1.+/,
        /.+?host2.+/
      ]);
    ...
    it('should respect `whitelistUrls`', function() {
      ...
      Raven._globalOptions.whitelistUrls = joinRegExp([
        /.+?host1.+/,
        /.+?host2.+/
      ]);
      Raven._processException('Error', 'error', 'http://host1/', []);
  ...
  describe('send', function() {
    ...
    it('should truncate url in breadcrumb', function() {
      ...
      var obj = {
        method: 'POST',
        url: 'http://example.org/api/0/auth/' + longUrl
      };
...
describe('Raven (public API)', function() {
  ...
  describe('callback function', function() {
    it('should callback a function if it is global', function() {
      ...
      assert.equal(
        Raven._globalEndpoint,
        'http://some.other.server:80/api/2/store/'
      );
  ...
  describe('.setUserContext', function() {
    ...
    it('should not merge globalContext.user object, but rewrite', function() {
      ...
      assert.deepEqual(Raven._globalContext.user, {
        age: 34,
        email: 'john@mail.com'
      });
    });
